...
// New 显式收 jwt 参数而非整个 *config.Config：service 不依赖 config 包，测试更易构造。
func New(repo *repository.Repository, rdb *redis.Client, jwtSecret string, jwtTTL time.Duration) *Service {
	return &Service{
		repo:      repo,
		rdb:       rdb,
