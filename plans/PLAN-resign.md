...
> **2026-08-19 列表恢复 dept_id 筛选（回滚上一条的部分结论）**：真实接口确认 `teacher/resign/list` 实际**支持 `dept_id`**（按原老师部门筛选），上一变更记录中「无 dept_id」判断有误。变更：`ResignListReq`/`ResignListFilter` 加回 `DeptID`（FlexInt64 宽容解析，`''`/`null`/缺省不过滤），repository `resignWhere` 恢复 `r.original_teacher_dept_id = ?` 精确匹配（对部门快照列，离职后部门调整不影响历史记录）；姓名三参数模糊筛选保持上一条不变。前端 teacherQuery.vue：`resignQuery` 加回 `dept_id`，部门树点击恢复联动离职 tab（与老师 tab 同步刷新）。README/Swagger 同步。

> **2026-08-20 列表移除「操作ip」展示**：前端不再展示操作 IP。变更（用户确认**保留 DB 列与历史数据**）：`Resign` 删 `OperateIP` 字段（列表响应不再含 `operate_ip` 键，Swagger 随 swag 重新生成消失）；`ListResigns` SELECT/Scan 删该列；`ResignInsert.OperateIP` 保留，add 仍写 `c.ClientIP()` 作审计留痕；schema 列保留、无迁移。前端 teacherQuery.vue 删「操作ip」表格列；teacherQuery-plan.md / teacherQuery-table-schema.md 同步。README 同步。
